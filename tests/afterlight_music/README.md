...
* `surface.DrawPoly` помечается нарисованным, только если текстура сброшена
  (`draw.NoTexture`): как в GMod, полигон рисуется текущей текстурой, и после
  `draw.RoundedBox` без сброса он был бы невидим — так стенд ловит «пустую
  иконку»;
* `cookie`, `timer`, `hook`, `input`, виртуальные часы и покадровый насос.

Покрытые сценарии (146 проверок):
