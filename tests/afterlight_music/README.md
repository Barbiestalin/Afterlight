...
* из папки плагина Helix грузит только `sh_plugin.lua`, а `cl_`-файлы плагин
  подключает сам через `ix.util.Include`.
...
* переход заставка → меню персонажей без перезапуска трека и без провала
  громкости, ползунок переезжает;
* выбор персонажа: затухание, канал ставится на паузу, позиция сохраняется;
* TAB: тот же канал снимается с паузы, ползунок справа внизу, отпускание TAB
  музыку не рвёт, закрытие TAB — плавное затухание;
* переход TAB → меню персонажей без остановки трека, ползунок уходит вместе со
  своим меню и появляется в новом;
* закрытие меню во время перетаскивания — захват мыши освобождается;
* `lua_refresh`: канал, громкость и единственный ползунок переживают
